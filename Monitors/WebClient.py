...
class WebClient(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        # TODO - find a way to deal with this, SBE jobs currently don't
        # trigger this check, but we need it for health checks
    ...
    def clientConnectionFailed(self, connector, reason):
        if self.params.debug:
            if "put" in self.op:
                ...
                sys.stderr.write("Job GET request clientConnectionFailed\t" % \
                    self.job.get_job_id())
...
class WebServiceCheckFactory(WebCoreFactory):
    ...
    def buildProtocol(self, addr):
        ...
        if self.authenticating:
            # This isn't technically true, but we're close enough, we
            # just needed to carry state to the WebClient() instance
            return WebClient(self,
                             verb="POST",
                             url=self.service.get_login_url(),
                             authing=True)
    ...
    def check_contents(self):
        if self.authenticating:
            ...
            # Why wait?  So we can collect cookies.
            # Otherwise, all requests go out instantly
            wait_for = 0.1
            waiting = 0
    ...
    def conn_pass(self, result):
        print "Job %s: Successfully connected to %s: %s" % \
            (self.get_job_id(), self.addr, result)
        self.service.pass_conn()

    def conn_fail(self, failure):
        # TODO content.get_url???
    ...
    def content_fail(self, failure, content):
        ...
        sys.stderr.write("Job %s: Failed content integrity check with result %s:  %s/%s | %s\n" % \
                         (self.job.get_job_id(),
                          failure,
                          self.service.get_port(),
                          self.service.get_proto(),
    ...
    def add_fail(self, reason):
        if "timeout" in reason:
            ...
            self.service.fail_conn(reason, "%s\r\n%s" % \
                (self.get_server_headers(), self.body))
